...
A REST API built with JAX-RS and Jersey for managing campus rooms and IoT sensors.
Module: 5COSC022W — Client-Server Architectures
Student ID: w21162895 | University of Westminster

API Overview
This API is the backend for the University Smart Campus project. It lets you manage rooms and the sensors inside them, things like temperature monitors, CO2 sensors and occupancy trackers.
Base URL
...
Tech Stack
-	Java 11+
-	JAX-RS with Jersey 2.39.1
-	Grizzly as the embedded HTTP server
-	Jackson for JSON
-	Maven for building
-	ConcurrentHashMap for in-memory storage, no database
...
cd smart-campus-api
2. Open in NetBeans
-	File > Open Project > pick the project folder
-	Right-click the project > Build
3. Run it
-	Right-click the project > Run
-	The server starts at http://localhost:8080/api/v1
-	Press ENTER in the output panel to stop it
...
6. Try deleting a room with sensors - expect 409
curl -X DELETE http://localhost:8080/api/v1/rooms/LIB-301
7. Try posting to a MAINTENANCE sensor - expect 403
curl -X POST http://localhost:8080/api/v1/sensors/OCC-001/readings -H "Content-Type: application/json" -d "{\"value\":10}"


Seeded Test Data
The API loads this data automatically when the server starts.
Rooms
ID	Name	Capacity
...
201 Created	Created	POST requests
204 No Content	Deleted	DELETE requests
400 Bad Request	Bad input	Missing fields
403 Forbidden	Blocked	POST to MAINTENANCE sensor
404 Not Found	Not found	Resource does not exist
409 Conflict	Conflict	Delete room that has sensors
422 Unprocessable Entity	Bad data	Sensor with fake roomId
...
├── SmartCampusApplication.java        # sets @ApplicationPath("/api/v1")
├── DataStore.java                     # holds all data in memory
├── model/
│   ├── Room.java
│   ├── Sensor.java
│   └── SensorReading.java
...
│   └── GlobalExceptionMapper.java     # catches everything, returns 500
└── filter/
...
In JAX-RS, every time a request comes in the framework creates a new instance of the resource class to handle it. Once the response is sent that instance is gone. This is called the request-scoped lifecycle and it is the default. So resource classes are not singletons and do not hold onto anything between requests.
This caused a problem for data storage. Since every resource object gets thrown away after each request, storing rooms or sensors inside those classes would mean losing the data every time. To get around this a separate DataStore class was created that holds everything in static fields. Static fields belong to the class itself not to any instance, so the data stays alive for as long as the server runs.
ConcurrentHashMap was used instead of a regular HashMap because multiple requests can hit the server at the same time across different threads. A normal HashMap is not built for that and can break or give back wrong results when two threads access it together. ConcurrentHashMap handles concurrent access safely without locking the whole thing, so it was the right choice.
